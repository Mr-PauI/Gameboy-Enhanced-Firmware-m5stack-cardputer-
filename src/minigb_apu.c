...
// Force definitions before including header
...
/* Define limits based on format */
    ...
    #define VOL_INIT_MAX        (1.0f/8.0f)
    #define VOL_INIT_MIN        (-1.0f/8.0f)
#endif
    ...
    audio_sample_t *stream)
{
    ...
    //memset(stream, 0, len);
    // Optimization: Assume stream is SAMPLES * 2 * sizeof(int16)
    memset(stream, 0, AUDIO_SAMPLES * 2 * sizeof(audio_sample_t));
