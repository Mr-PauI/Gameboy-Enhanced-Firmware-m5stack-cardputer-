...
Press \ to cycle through available display aspect ratios (note gameboy border hidden/off-screen when in 16:9 or above)
...
12.28.2024:v0.79.61
* Updated to latest version of Walnut-CGB
...
* revised 24 opcodes to take full advantage of new cpu core architecture(temporarily disabled)
...
* full screen(16:9) is now the default scaling mode
...
29.06.2024:v0.76
* Compatibility mode settings now take effect immediately if changed mid-game; will display message if mode is locked on due to override requirement (ie: Battlezone which requires it on for any functionality)
...
* No longer crashes when there are too many files, will list first 512 
...
19.05.2024:v0.72
* Implemented Default Palette selection
* Added Default Palette, border options
...
* Added Dragon Warrior I & II SGB & AP support 

11.06.2024(pm):v0.68
* Control A and Control Start will now also navigate into directories
* Message Box contrast increased (darker background)
...
* Added Missile Command/Asteroids SGB and AP support
* Added Space Invaders SGB & SP support
...
* Added SGB palette for Donkey Kong Land
...
* Added Pokemon Yellow  AP Community Palette
* Added Final Fantasy Legends SGB Palette, added 12 colour Analogue Pocket palette
...
02.06.2024:v0.622
* Added mute and volume half/max controls to Fn+Left/Right keys

01.05.2024:v0.621
* Experimented with an inline version of the ganeboy CPU its various functions but they yielded no performance gain
...
30.05.2024.v0.62
* Audio rendering has been made faster updating every 24 by default samples, was twelve; 24 will be the default (a.k.a. medium quality) sligt reduction in quality when frame rate dips low can be noticed at 36; may make option for audio quality. Updating every 36 samples seems to be the minimum(clicking becomes more likely beyond this update interval), so 12,24,36 seem to be appropriate settings; every 12 samples was used in 6.0 may added higher and lower quality settings in future
* Adjusted TTL logic to run separately from the page retrieval so that retrieving data is as fast as possible when in memory.

29.05.2024:v0.61
* Memory solution devised was based on two ideas: Assume statistically random access patterns, and place no limits on addressable space.
...
27.05.2024:v0.6
* Audio is fully implemented. Inaccurately, built for speed. Hardly any impact of rendering.
...
21.05.2024:v0.51
* implemented main emulator function as a task on core 1 and a second persistent task on core 0 via RTOS; this is in preparation to accommodate the APU engine
* core 0 thread must call vTaskDelay as there are vital threads on that core, which if not attended to, will cause a restart. I dont have full use of this core.
...
turn on gameboy border at any time by pressing Fn+'['
